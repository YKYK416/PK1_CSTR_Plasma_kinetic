...
PRIMARY_RUNNABLE_STATES = {"planned", "interrupted", "failed"}
NORMAL_WORKERS = 4
...
def restore_interrupted_duplicate_stationarity_attempts(conn: Any, root: Path) -> int:
    """Restore an interrupted duplicate 100 s retry to its prior valid result."""
    rows = conn.execute(
        """SELECT c.case_id,current_attempt.attempt_number AS duplicate_attempt,
                  prior.attempt_number AS source_attempt,prior.error_text AS source_error
           FROM cases c
           JOIN attempts current_attempt
             ON current_attempt.case_id=c.case_id AND current_attempt.attempt_number=c.active_attempt
           JOIN attempts prior ON prior.case_id=c.case_id
           WHERE c.status='interrupted'
             AND current_attempt.error_text='controller restart found no live recorded solver PID'
             AND prior.status='invalid_output'
             AND prior.error_text LIKE ?
             AND prior.attempt_number < current_attempt.attempt_number
           ORDER BY c.case_id,prior.attempt_number DESC""",
        (STATIONARITY_PREFIX + "%",),
    ).fetchall()
    restored = 0
    seen: set[str] = set()
    ...
    for row in rows:
        ...
        seen.add(case_id)
        conn.execute(
            """UPDATE cases SET status='invalid_output',active_attempt=?,last_error=?,updated_at=?
               WHERE case_id=?""",
            (int(row["source_attempt"]), row["source_error"], now, case_id),
        )
        ledger.append_event(
            conn, root, "duplicate_stationarity_attempt_restored",
            {"interrupted_duplicate_attempt": int(row["duplicate_attempt"]),
             "restored_stationarity_attempt": int(row["source_attempt"]),
             "reason": "stationarity failures may only re-enter via the 200 s extension phase"},
            {"case_id": case_id}, int(row["source_attempt"]),
        )
        restored += 1
    conn.commit()
    return restored
...
def run_campaign(root: Path, phase: str) -> None:
    ...
    try:
        with ledger.campaign_lock(root):
            recovered = ledger.recover_orphaned(conn, root)
            restored_duplicates = restore_interrupted_duplicate_stationarity_attempts(conn, root)
            ledger.append_event(conn, root, "campaign_run_started", {
                "phase": phase,
                "recovered_orphaned": recovered,
                "restored_duplicate_stationarity_attempts": restored_duplicates,
                "residence_time_s": TAU_RES_S,
            })
            conn.commit()
            ...
            original_runnable_states = ledger.RUNNABLE_STATES
            try:
                # A completed 100 s stationarity failure is not eligible for
                # another ordinary 100 s attempt.  It is promoted exactly once
                # by schedule_200s_extensions() after stage 1 finishes.
                ledger.RUNNABLE_STATES = PRIMARY_RUNNABLE_STATES
                for current_phase in phases:
                    workers = 1 if current_phase.startswith("high_stiffness_") else NORMAL_WORKERS
                    ledger.run_phase(conn, root, work, current_phase, workers)
                    ledger.export_overview(conn, root)
                    write_live_ledger(conn, root)
            finally:
                ledger.RUNNABLE_STATES = original_runnable_states
            if phase == "all":
